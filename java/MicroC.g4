...
var_decl : base_type ident ';' {st.addVariable($base_type.t, $ident.text);};

str_decl : 'string' ident '=' val= STR_LITERAL ';' {st.addVariable(Scope.Type.STRING, $ident.text, $val.text);};

base_type returns [Scope.Type t]: 'int' {$t = Scope.Type.INT;}| 'float' {$t = Scope.Type.FLOAT;};

/* Functions */

func_decl : base_type ident '(' params ')' ';' {st.addFunction($base_type.t, $ident.text, $params.types);};
...
function returns [FunctionNode node] : base_type ident '(' params ')' 
      {
          ...
          if ((ste == null) || !ste.isDefined()) {
               st.addFunction($base_type.t, $ident.text, $params.types);          
...
param returns [String name, Scope.Type type] : base_type ident {$name = $ident.text; $type = $base_type.t;};                   
