...
cast_expr returns [CastNode node] : '(' type ')' expr {$node = new CastNode($expr.node, $type.t);};
...
/*
Original Attempt:
array_expr returns [PtrDerefNode node] : lval '[' expr ']' {$node = new PtrDerefNode($expr.node);} 
        | ae=array_expr '[' expr ']'{$node = new PtrDerefNode($expr.node);}; 
*/
/* New Attempt: */
array_expr returns [PtrDerefNode node] : lval '[' expr ']' 
     {
          /*lval + 4 * expr */
     $node = new PtrDerefNode(new BinaryOpNode($lval.node, new BinaryOpNode($expr.node, new IntLitNode("4"), "*"), "+"));
     } 
     | ae=array_expr '[' expr ']'
     {$node = new PtrDerefNode($expr.node); 
     };
