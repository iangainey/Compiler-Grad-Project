...
The Intermediate Representation (IR) generated by this compiler is 3 Address Code (3AC).  
The IR is a similiar form to the assembly generated at the end, although where the assembly is machine specific, the generated IR is not.  
The 3AC IR has at most 3 operands, with 2 source operands and a single destination operand. It does not use any registers, only temporaries. It does not differentiate in the instructions between literals, temporaries, and variables aside from the name prefix.  
The naming prefix convention assigned to operands in the 3AC IR are:
- *$t* corresponds to an integer temporary
- *$f* corresponds to a float temporary
- *$g* corresponds to a global variable
- *$l* corresponds to a local variable  
This 3AC is generated in the CodeGenerator.java in the Register Allocation branch of this repository. The LocalRegisterAllocator.java file is where a macro expansion converts this Intermediate Representation into RISC-V Assembly through performing register allocation

### Register Allocation
This compiler supports Local Register Allocation, which can be seen in the Register Allocation branch of this repository.  
This is accomplished in the java/assembly/LocalRegisterAllocator.java file, with the first step being splitting the 3AC into basic blocks. Each basic block has no control flow, and is guaranteed to execute all the instructions in that basic block. The purpose of this is to allow register allocation to be performed for each basic block, simplifying the analysis due to eliminating any opportunity for control flow to cause confusion.  

Once the 3AC has been splot into basic blocks, the allocateBasicBlock method converts the 3AC in each basic block into RISC-V Assembly, and performs register allocation. This can be seen beginning at line 109.

To convert the 3AC into assembly, it undergoes a macro expansion beginning at list 225 in the macroExpand3O class.  In this step of the process, the 3AC instruction is expanded into a RISC-V instruction set. The corresponding class to the 3AC instruction handles any loading, storing, and moving required for that instruction that the 3AC does not cover.  

The macro expansion classes for each instruction also perform register allocation. This is accomplished by:
- Ensuring the required data is stored in a register. This generates a call to the ensureSource function on line 457, or the ensureDest function on line 544, depending on the operation. These functions first find if any register is currently holding this data. 
  * If it is, it returns this register. 
- If no register is holding this data, it finds a register that is available. 
  * If one is available, it loads the data into this register.
  * If none are available, it frees up a register to be used. Then, it loads the data into this register.
Once register allocation is complete, the instruction set is added to the instructions returned to the CodeGenerator.

The list of registers that were available for the machine the tests were run on were:
- Integer Registers x1 through x31
  * x1 is reserved for the return address, ra
  * x2 is reserved for the stack pointer, sp
  * x8 is reserved for the frame pointer, fp
- Floating Pointer Registers f0 through f31
  
The RegisterFile class does allow for limiting the registers to different quantites and availability depending on the machine. This can be done by modifying the numIntRegisters and numFloatRegisters variables in the CodeGenerator class.
